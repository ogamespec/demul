...
bool CheckPad(HWND hwnd, int param, int joyIdx) {
	if (CheckButton(hwnd, param, padControls[joyIdx].ButtonUp, padControls[joyIdx].EditUp, &tunePadCfg.joy[joyIdx].UP))
		return true;
	if (CheckButton(hwnd, param, padControls[joyIdx].ButtonDown, padControls[joyIdx].EditDown, &tunePadCfg.joy[joyIdx].DOWN))
		return true;
	if (CheckButton(hwnd, param, padControls[joyIdx].ButtonLeft, padControls[joyIdx].EditLeft, &tunePadCfg.joy[joyIdx].LEFT))
		return true;
	if (CheckButton(hwnd, param, padControls[joyIdx].ButtonRight, padControls[joyIdx].EditRight, &tunePadCfg.joy[joyIdx].RIGHT))
		return true;
	if (CheckButton(hwnd, param, padControls[joyIdx].ButtonA, padControls[joyIdx].EditA, &tunePadCfg.joy[joyIdx].A))
		return true;
	if (CheckButton(hwnd, param, padControls[joyIdx].ButtonB, padControls[joyIdx].EditB, &tunePadCfg.joy[joyIdx].B))
		return true;
	if (CheckButton(hwnd, param, padControls[joyIdx].ButtonC, padControls[joyIdx].EditC, &tunePadCfg.joy[joyIdx].C))
		return true;
	if (CheckButton(hwnd, param, padControls[joyIdx].ButtonD, padControls[joyIdx].EditD, &tunePadCfg.joy[joyIdx].D))
		return true;
	if (CheckButton(hwnd, param, padControls[joyIdx].ButtonX, padControls[joyIdx].EditX, &tunePadCfg.joy[joyIdx].X))
		return true;
	if (CheckButton(hwnd, param, padControls[joyIdx].ButtonY, padControls[joyIdx].EditY, &tunePadCfg.joy[joyIdx].Y))
		return true;
	if (CheckButton(hwnd, param, padControls[joyIdx].ButtonZ, padControls[joyIdx].EditZ, &tunePadCfg.joy[joyIdx].Z))
		return true;
	if (CheckButton(hwnd, param, padControls[joyIdx].ButtonLTrig, padControls[joyIdx].EditLTrig, &tunePadCfg.joy[joyIdx].LTRIG))
		return true;
	if (CheckButton(hwnd, param, padControls[joyIdx].ButtonRTrig, padControls[joyIdx].EditRTrig, &tunePadCfg.joy[joyIdx].RTRIG))
		return true;
	if (CheckButton(hwnd, param, padControls[joyIdx].ButtonStart, padControls[joyIdx].EditStart, &tunePadCfg.joy[joyIdx].START))
		return true;
	if (CheckButton(hwnd, param, padControls[joyIdx].ButtonS1Up, padControls[joyIdx].EditS1Up, &tunePadCfg.joy[joyIdx].STICK1UP))
		return true;
	if (CheckButton(hwnd, param, padControls[joyIdx].ButtonS1Down, padControls[joyIdx].EditS1Down, &tunePadCfg.joy[joyIdx].STICK1DOWN))
		return true;
	if (CheckButton(hwnd, param, padControls[joyIdx].ButtonS1Left, padControls[joyIdx].EditS1Left, &tunePadCfg.joy[joyIdx].STICK1LEFT))
		return true;
	if (CheckButton(hwnd, param, padControls[joyIdx].ButtonS1Right, padControls[joyIdx].EditS1Right, &tunePadCfg.joy[joyIdx].STICK1RIGHT))
		return true;
	if (CheckButton(hwnd, param, padControls[joyIdx].ButtonS2Up, padControls[joyIdx].EditS2Up, &tunePadCfg.joy[joyIdx].STICK2UP))
		return true;
	if (CheckButton(hwnd, param, padControls[joyIdx].ButtonS2Down, padControls[joyIdx].EditS2Down, &tunePadCfg.joy[joyIdx].STICK2DOWN))
		return true;
	if (CheckButton(hwnd, param, padControls[joyIdx].ButtonS2Left, padControls[joyIdx].EditS2Left, &tunePadCfg.joy[joyIdx].STICK2LEFT))
		return true;
	if (CheckButton(hwnd, param, padControls[joyIdx].ButtonS2Right, padControls[joyIdx].EditS2Right, &tunePadCfg.joy[joyIdx].STICK2RIGHT))
		return true;
...
static BOOL CALLBACK Configure(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
	...
	case WM_INITDIALOG:
	{
		for (i = 0; i < MAX_JOYS; i++)
			UpdatePadEdits(hwnd, &tunePadCfg.joy[i], &padControls[i]);
	...
	case WM_TIMER:
		if (enableTimer) {
			...
			if (Key) {
				enableTimer = 0;
				for (i = 0; i < MAX_JOYS; i++) {
					RemoveUsedKey(Key, &tunePadCfg.joy[i]);
					UpdatePadEdits(hwnd, &tunePadCfg.joy[i], &padControls[i]);
				}
				*key = Key;
...
bool padLoadConfig(bool autoSetConfig) {
	IniFile iniFile;
	char padName[10];
	int i;

	memset(&padcfg, 0, sizeof(padcfg));
	...
	for (i = 0; i < MAX_JOYS; i++) {
		...
		IniFile_LoadJoy(&iniFile, padName, &padcfg.joy[i]);
	}
	CalcJoyCaps();
	return true;
}
...
void padSaveConfig() {
	...
	for (i = 0; i < MAX_JOYS; i++) {
		sprintf(padName, "PAD%i", i);
		IniFile_SaveJoy(&iniFile, padName, &padcfg.joy[i]);
	}
}
...
void CalcJoyCaps() {
	...
	for (i = 0; i < MAX_JOYS; i++) {
		...
		if (padcfg.joy[i].UP != 0)
			caps |= CONT_DPAD_UP;
		if (padcfg.joy[i].DOWN != 0)
			caps |= CONT_DPAD_DOWN;
		if (padcfg.joy[i].LEFT != 0)
			caps |= CONT_DPAD_LEFT;
		if (padcfg.joy[i].RIGHT != 0)
		...
		if (padcfg.joy[i].Z != 0)
			caps |= CONT_Z;
		if ((padcfg.joy[i].STICK1UP != 0) && (padcfg.joy[i].STICK1DOWN != 0))
			caps |= CONT_STICK1Y;
		if ((padcfg.joy[i].STICK1LEFT != 0) && (padcfg.joy[i].STICK1RIGHT != 0))
			caps |= CONT_STICK1X;
		if ((padcfg.joy[i].STICK2UP != 0) && (padcfg.joy[i].STICK2DOWN != 0))
			caps |= CONT_STICK2Y;
		if ((padcfg.joy[i].STICK2LEFT != 0) && (padcfg.joy[i].STICK2RIGHT != 0))
			caps |= CONT_STICK2X;
		if (padcfg.joy[i].LTRIG != 0)
			caps |= CONT_LEFT_TRIG;
		if (padcfg.joy[i].RTRIG != 0)
