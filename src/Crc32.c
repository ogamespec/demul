...
u32 FASTCALL CalcCrc32(const u8*aBuf, u32 aSize) {
	register u32 i;
	register u32 crc32 = 0xFFFFFFFF;
	for (i = 0; i < aSize; i++)
		crc32 = _crc32[((u8)(crc32 ^ ((u32)aBuf[i])))] ^ (crc32 >> 8);
	return ~crc32;
}

u32 FASTCALL CalcCrc32cont(u32 crc, const u8*aBuf, u32 aSize) {
	register u32 i;
	register u32 crc32 = ~crc;
	for (i = 0; i < aSize; i++)
		crc32 = _crc32[((u8)(crc32 ^ ((u32)aBuf[i])))] ^ (crc32 >> 8);
	return ~crc32;
}
...
u32 FASTCALL CalcChecksum32(const u8 *aBuf, u32 aSize) {
	register u32 i, a = 0, b = 0;
	register u32 checksum = 0xFFFFFFFF;
	for (i = 0; i < aSize; i += 4) {
		a += aBuf[i + 0];
		b += aBuf[i + 1];
		checksum += a + b;
